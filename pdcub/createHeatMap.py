...
def createCodonLabelsChem(codonNamesChem,codonNamesAlpha,lw,ax):
    ...
    for l in incs:
        ...
        ax.text(-.15,midPos,l[0],fontsize=7,rotation=90,fontweight = 'light',transform=ax.transAxes,clip_on=False)
...
def heatmap(heatmapType, data,row_labels, col_labels, ax=None,cbar_kw={},cbarlabel='', **kwargs):
   ...
   im = ax.imshow(data,norm=norm, **kwargs, aspect="auto")
   ...
   ax.set_xticklabels(np.arange(0,3000,300),fontsize = 6)
   ax.set_xlabel("Distance from Start Codon (nt)", fontsize = 8)
...
def main():
    ...
    #plt.pcolor(data, edgecolors='k', linewidths=0.3)

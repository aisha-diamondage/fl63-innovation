...
def parseDefline(record):
    ...
    length = end
    code = str(record.id).split('|')[1].split("_cds_")[0]
    gene= str(record.description).split("gene=")[1].split("]")[0]
    end = len(transcript)
    #print("THIS IS THE ID", start,end,code,gene,length)
...
def processFastaFile(fastaFile):
    ...
    for record in SeqIO.parse(fastaFile, "fasta"):
        ...
        if transcript[start-1:start+2] != 'ATG': continue
        if transcript[end-3:end] not in ['TAA','TAG','TGA']: continue
...
def buildScoreFile(logData,fastaFile,numBins,binSize,modelType):
    ...
    for transcript in transcripts:
        ...
        for match in re.finditer('ATG',str(transcript)):
            index = match.start()

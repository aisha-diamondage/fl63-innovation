...
def getGC(valid_mRNAs):
    ...
    #print(valid_mRNAs, totalNt)
    #print(firstSegmentNt, firstSegmentGc)
    for mRNA in valid_mRNAs:
        ...
        #print("first", firstSegmentGcSingleRatio, singleCdsGcRatiosSeg1[shortID])
...
def getCodonFrequencies(valid_mRNAs):
    ...
    codonBinTotal = []
    codonBinPositional = {}
    #print(binSizeNt*binCount)
    for codonPosition in range(0,int(binSizeNt*binCount/3)):
    ...
    for codon in codonMap:
        ...
        for codonPosition in range(int(binSizeNt*binCount/3)):
...
def calculateRelativeAdaptiveness(codonFrequenciesOne,codonFrequenciesTwo,codonFrequenciesTotal,codonFrequenciesPerPosition,codonFrequenciesPerPositionCumulative):
    ...
    for position in range(0,int(codonSequenceLength)):
...
def calculateCAIs(valid_mRNAs,RA1group,RA2group,RAtotalgroup,cumulativePositionalRAgroup):
    ...
    for mRNA in valid_mRNAs:
        ...
        for i in range(0,binSizeNt,3):
            #print(i, transcriptCumulativePositionalCodons[shortID])
            codon = cds[i:i+3]
        ...
        for i in range(0,2*binSizeNt,3):
            ...
            if i>0:
                ...
                transcriptCumulativePositionalCodons[shortID][i/3] = transcriptCumulativePositionalCodons[shortID][(i-3)/3] + transcriptCumulativePositionalCodons[shortID][i/3]
...
for ID in IDs:
    ...
    try:
        ...
        print("this ID did not passs: ", ID)
...
plt.scatter(caiSegOneVals,caiSegTwoVals,c=cols,s=5,cmap=colorMap,edgecolor='none',alpha=0.8)
